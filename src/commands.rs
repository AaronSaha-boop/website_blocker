// src/commands.rs
//
// Tauri command wrappers — each function builds a ClientMessage, sends it to
// the daemon via client::send_message, and returns structured JSON.

use serde_json::{json, Value};

use crate::client;
use crate::db::{BlockedApp, BlockedWebsite, DomRule, Schedule};
...
pub async fn add_global_website(domain: String) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::AddGlobalWebsite { domain }).await?;
...
pub async fn remove_global_website(domain: String) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::RemoveGlobalWebsite { domain }).await?;
...
pub async fn create_schedule(
    ...
    let days_vec: Vec<String> = days.split(',').map(|d| d.trim().to_lowercase()).collect();
...
pub async fn list_schedules(profile_id: String) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::ListSchedules { profile_id }).await?;
    ...
    match response {
        DaemonMessage::ActivePolicy(p) => Ok(json!({
            ...
            "dom_rules": p.dom_rules.into_iter().map(|r| json!({
                "id": r.id, "site": r.site, "toggle": r.toggle,
            })).collect::<Vec<_>>(),
        })),
        other => Err(format!("Unexpected response: {:?}", other)),
    }
...
pub async fn add_blocked_website(profile_id: String, domain: String) -> Result<Value, String> {
    let response = client::send_message(ClientMessage::CreateBlockedWebsite {
        website: BlockedWebsite { id: 0, profile_id, domain },
    })
    .await?;

    match response {
        DaemonMessage::BlockedWebsite(w) => Ok(json!({
            "id": w.id, "profile_id": w.profile_id, "domain": w.domain,
        })),
        other => Err(format!("Unexpected response: {:?}", other)),
    }
}
...
pub async fn list_blocked_websites(profile_id: String) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::ListBlockedWebsites { profile_id }).await?;

    match response {
        DaemonMessage::BlockedWebsiteList(websites) => {
            let list: Vec<Value> = websites
                .into_iter()
                .map(|w| json!({ "id": w.id, "profile_id": w.profile_id, "domain": w.domain }))
                .collect();
            Ok(json!(list))
        }
        other => Err(format!("Unexpected response: {:?}", other)),
...
pub async fn delete_blocked_website(id: i64) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::DeleteBlockedWebsite { id }).await?;
...
pub async fn add_blocked_app(
    ...
    let response = client::send_message(ClientMessage::CreateBlockedApp {
        app: BlockedApp { id: 0, profile_id, app_identifier },
    })
    .await?;

    match response {
        DaemonMessage::BlockedApp(a) => Ok(json!({
            "id": a.id, "profile_id": a.profile_id, "app_identifier": a.app_identifier,
        })),
        other => Err(format!("Unexpected response: {:?}", other)),
    }
}
...
pub async fn list_blocked_apps(profile_id: String) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::ListBlockedApps { profile_id }).await?;
    ...
    match response {
        DaemonMessage::BlockedAppList(apps) => {
            let list: Vec<Value> = apps
                .into_iter()
                .map(|a| {
                    json!({ "id": a.id, "profile_id": a.profile_id, "app_identifier": a.app_identifier })
                })
                .collect();
            Ok(json!(list))
        }
...
#[tauri::command]
pub async fn add_dom_rule(
    profile_id: String,
    site: String,
    toggle: String,
) -> Result<Value, String> {
    let response = client::send_message(ClientMessage::CreateDomRule {
        rule: DomRule { id: 0, profile_id, site, toggle },
    })
    .await?;

    match response {
        DaemonMessage::DomRule(r) => Ok(json!({
            "id": r.id, "profile_id": r.profile_id, "site": r.site, "toggle": r.toggle,
        })),
        other => Err(format!("Unexpected response: {:?}", other)),
    }
}
...
pub async fn list_dom_rules(profile_id: String) -> Result<Value, String> {
    let response =
        client::send_message(ClientMessage::ListDomRules { profile_id }).await?;
    ...
    match response {
        DaemonMessage::DomRuleList(rules) => {
            let list: Vec<Value> = rules
                .into_iter()
                .map(|r| {
                    json!({ "id": r.id, "profile_id": r.profile_id, "site": r.site, "toggle": r.toggle })
                })
                .collect();
            Ok(json!(list))
        }
